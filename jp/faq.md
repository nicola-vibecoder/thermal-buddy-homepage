...
- [データと同期](#データと同期)
  ...
  - [iCloud 同期のオン／オフを切り替えるには？](#icloud-同期のオンオフを切り替えるには)
...
### まずアクティビティとギアカテゴリーをカスタマイズしよう

最初のログを記録する前に、**Settings** で少し時間をかけてアプリを自分仕様に整えておくことをおすすめします。アクティビティやギアカテゴリーは名前の変更・非表示・並び替え・追加が可能で、アクティビティにはアイコンも設定できます。最初に設定を整えておくと、以降のすべてのログ入力がスムーズで自然に感じられます。
...
ギアアイテムを追加するとき、アイテム名のフィールドに入力を始めると、過去に記録したギアが候補として表示されます。タップするだけでブランドとカテゴリーが自動入力され、手入力の手間がありません。
...
### iCloud 同期のオン／オフを切り替えるには？

iCloud 同期の設定は、Thermal Buddy アプリ内ではなく、iPhone の「設定」アプリから行います。

**切り替え方法：**

1. iPhone の**設定**アプリを開く
2. 画面上部の自分の名前（Apple アカウント）をタップ
3. **iCloud** をタップ
4. **iCloudを使用しているApp** をタップ（**すべてを表示**が必要な場合あり）
5. **Thermal Buddy** を探してトグルをオン／オフ

同期をオフにすると、ログは現在のデバイスにのみ保存され、同じ iCloud アカウントにサインインしている他の Apple デバイスには表示されません。
...
- **設定 → [お名前] → iCloud → iCloudを使用しているApp** で Thermal Buddy が有効になっている（[iCloud 同期のオン／オフを切り替えるには？](#icloud-同期のオンオフを切り替えるには) を参照）
