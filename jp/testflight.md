...
### 最初に設定でアクティビティとカテゴリを自分仕様に整えておく

最初のログを書く前に、**設定**画面で少しだけ準備しておくと、その後の記録がグッと楽になります。アクティビティやギアカテゴリは、名前の変更・非表示・並べ替え・追加ができます。アクティビティはアイコンの変更も可能です。自分の使い方に合った構成にしておくと、毎回のログ入力がスムーズになります。

### アイテム名を入力すると候補が出て、ブランドとカテゴリが自動で入る

ギアアイテムを追加するとき、名前の欄に文字を打ち始めると、過去に記録したギアが候補として表示されます。候補をタップするだけで、ブランドとカテゴリが自動的に入力されます。毎回すべて入力し直す必要はありません。
...
- ✅ **マイギア** — これまでのログに登場したすべてのギアをカテゴリ別に一覧表示する専用画面が追加されました。検索・並べ替え（カテゴリ・ブランド・名前）に対応し、アイテムをタップするとその履歴を確認できます。山行前にカタログとしてギアを登録することもでき、まだログに登場していないアイテムには「未使用」バッジが表示されます。ホーム画面のすべてのタイルがタップ可能になりました。
- ✅ **ログの複製** — ログカードを左スワイプするか、詳細画面のツールバーから複製を実行すると、編集モードで複製が開きます。フリープランの上限が適用されます。

今後の予定は[ロードマップ](roadmap.md)をご覧ください。

---

## Build 6 の新機能

- ✅ **未保存のまま閉じる前に確認** — シーンに入力が残っている状態で閉じようとすると、先に確認が表示されます。保存するか、意図的に破棄するかを選べるため、誤タップで丁寧に入れた内容を失いにくくなりました。

---

## Build 5 の新機能

前のビルドで「近日対応予定」としていた項目が、すべて今回のビルドに含まれています。

- ✅ **気温ピッカーの改善** — 最低・最高を同じ値に設定できるようになりました。また、片方を動かしすぎても予期せず弾き返されることなく、もう片方がついてくるようになりました。
- ✅ **気温の直接入力を長押しに変更** — ホイール中央の「長押し」でキーボードが開くようになりました。スクロール中の誤作動がなくなります。
- ✅ **シーンをまたいだギアの候補表示** — 同じログの記録中に、前のシーンで入力したギアが次のシーンでも候補として表示されるようになりました。
- ✅ **ギア名の部分一致** — 名前の先頭だけでなく、途中からでも一致する候補が表示されます。先頭一致は引き続き優先されます。
- ✅ **ブランド名からギアを候補表示** — ブランド名を入力すると、そのブランドの過去のギアが候補として表示されます。アイテム名を打ち始める前から候補が出ます。
- ✅ **ギアアイテムをその場で編集** — ギアアイテムをタップして名前・ブランド・カテゴリをその場で修正できます。ログ作成中でも、あとからログブックでも使えます。
