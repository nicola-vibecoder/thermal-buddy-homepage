...
近いうちにリリース予定の改善内容です。テスト中にこれらの使いにくさを感じた場合は、すでに対応中であることをご承知おきください。

- **気温ピッカー** — 最低・最高を同じ値に設定できない点、および片方を動かしすぎると予期せず弾き返される点を修正します。
- **気温の直接入力を長押しに変更** — ホイール中央のタップでキーボードが開く操作を、誤作動しにくい「長押し」に変更します。
- **シーンをまたいだギアの候補表示** — 同じログの記録中に、前のシーンで入力したギアが次のシーンでも候補として表示されるようになります。
- **ギア名の部分一致検索** — 名前の先頭だけでなく、途中からでも一致する候補が表示されるようになります。
- **ブランド名からギアを候補表示** — ブランド名を入力すると、そのブランドの過去のギアが候補として表示されます。
- **ギアアイテムをその場で編集** — ギアアイテムの修正に、削除して登録し直す必要がなくなります。名前・ブランド・カテゴリのいずれも、ログ作成中にも、あとからログブックでも、タップして編集できるようになります。
